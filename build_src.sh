...
if [ -z "$lowfi_VERSION" ] || [ -z "$BUILD_VERSION" ]; then
    echo "Usage: $0 <lowfi_version> <build_version>"
    echo "Example: $0 0.5.0 1"
    exit 1
fi

PACKAGE_NAME="lowfi"
ORIG_TARBALL="${PACKAGE_NAME}_${lowfi_VERSION}.orig.tar.gz"
BUILD_DIR="${PACKAGE_NAME}-${lowfi_VERSION}"

echo "Creating Debian/Ubuntu source packages for lowfi ${lowfi_VERSION}-${BUILD_VERSION}..."
...
build_source_package() {
    local dist=$1
    local FULL_VERSION="${lowfi_VERSION}-${BUILD_VERSION}~${dist}"
    ...
    rm -rf "$BUILD_DIR"
    tar -xf "$ORIG_TARBALL"
    # GitHub archives extract as lowfi-0.x.y/ which matches our BUILD_DIR
...
lowfi (${FULL_VERSION}) ${dist}; urgency=medium

  * New upstream release ${lowfi_VERSION}.
