...
do
  ...
  docker build . -f Dockerfile.ubu -t lowfi-ubuntu-$UBUNTU_DIST --build-arg UBUNTU_DIST=$UBUNTU_DIST --build-arg lowfi_VERSION=$lowfi_VERSION --build-arg BUILD_VERSION=$BUILD_VERSION --build-arg FULL_VERSION=$FULL_VERSION
